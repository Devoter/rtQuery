...
namespace Rt\Storage {
	...
	class StorageObject extends \Rt\Storage\AbstractStorageObject {
		...
		/**
		 * 
		 * Ссылка на идентификатор соединения с СУБД
		 * @var unknown_type
		 */
		private $_db;
		 ...
		 */
		public function get($propertyName, $index = 0, $queryLine = true, $typecast = false)
		{
			if(isset($this->_properties[$propertyName]) && isset($this->_properties[$propertyName]['values'][$index])) {
				if($queryLine) {
					$ret = " `".$this->_table."`.`".$this->_properties[$propertyName]['realName']."` ".$this->_properties[$propertyName]['values'][$index]['exp']." ";
		...
		public function set($propertyName, $expression, $value, $index = 0)
		{
			if(isset($this->_properties[$propertyName]) && (
				...
				elseif(($expression == "LIKE") || ($expression == "REGEXP"))
					...
					$this->_properties[$propertyName]['values'][$index]['val'] = mysqli_real_escape_string($this->_db, $value);
				return true;
			}
			return false;
		}
			...
			if(is_array($data) && isset($data['expression']) &&
				...
				if(($data['expression'] == "=") || ($data['expression'] == "<>") ||
					...
					elseif(($data['expression'] == "LIKE") || ($data['expression'] == "REGEXP"))
						...
						$this->_properties[$propertyName]['values'][$index]['val'] = mysqli_real_escape_string($this->_db, $data['value']);
				}
			}
			return false;
		}
		
		public function unset($propertyName, $index = 0)
		{
			if(isset($this->_properties[$propertyName]) && isset($this->_properties[$propertyName]['values'][$index])) {
